...
- Für End-to-End Tests nutzen wir [Playwright](https://playwright.dev/docs/intro). Die Konfiguration ist bereits im Repository eingerichtet. (Secrets in den githib settings müssen noch rein)
- Folgende Befehle stehen zur Verfügung:
  - `npx playwright test` - Führt alle End-to-End Tests aus
  - `npx playwright test --ui` - Startet den interaktiven UI-Modus
  - `npx playwright test --project=chromium` - Führt Tests nur auf Desktop Chrome aus
  - `npx playwright test example` - Führt Tests in einer bestimmten Datei aus
  - `npx playwright test --debug` - Führt Tests im Debug-Modus aus
  - `npx playwright codegen` - Generiert Tests automatisch mit Codegen
