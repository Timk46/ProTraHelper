...
export class VotingMechanismDialogComponent {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { currentVotes: number; maxVotes: number }
  ) {}
  ...
  getProgressPercentage(): number {
    if (this.data.maxVotes === 0) return 0;
    const usedVotes = this.data.maxVotes - this.data.currentVotes;
