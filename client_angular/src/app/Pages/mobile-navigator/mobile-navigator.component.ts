...
export class MobileNavigatorComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    this.graphDataService.fetchUserGraph(this.currUserModule).subscribe({
      next: (data: ConceptGraphDTO) => {
        if (!data || !data.nodeMap || Object.keys(data.nodeMap).length === 0) {
        ...
        if (data.nodeMap[targetNodeId]) {
          ...
          if (targetNode.childIds.length === 0 && targetNode.parentIds.length > 0 && data.nodeMap[targetNode.parentIds[0]]) {
          ...
          console.warn(`Target node ${targetNodeId} not found in graph data. Falling back to default logic.`);
          if (data.currentConceptId && data.currentConceptId !== -1 && data.nodeMap[data.currentConceptId]) {
            initialNodeId = data.currentConceptId;
            const initialNode = data.nodeMap[initialNodeId];
            if (initialNode.childIds.length === 0 && initialNode.parentIds.length > 0 && data.nodeMap[initialNode.parentIds[0]]) {
            ...
            console.log(`CurrentConceptId invalid or missing (${data.currentConceptId}), falling back to trueRootId: ${initialNodeId}`);
          } else {
              console.error("Could not determine a valid initial node. Both currentConceptId and trueRootId are invalid or missing, and target node 2 was not found.");
  ...
  private performSearch(searchTerm: string): void {
    ...
    if (!this.userGraphData || !this.userGraphData.nodeMap) {
        this.searchResults = [];
        return;
    }
  ...
  navigateToSearchResult(node: ConceptNodeDTO): void {
    if (!this.userGraphData.nodeMap[node.databaseId]) {
      console.error(`Cannot navigate to search result: Node ${node.databaseId} not found in graph.`);
      return;
    }
    ...
    } else if (targetNode.parentIds.length > 0 && this.userGraphData.nodeMap[targetNode.parentIds[0]]) {
  ...
  addOpenedLayer(nodeId: number): void {
    if (this.userGraphData.nodeMap[nodeId]) {
      ...
      if (this.openLayers.findIndex(layer => layer.id === nodeId) === -1) {
          ...
          console.log("Added Layer:", nodeId, "Open Layers:", this.openLayers.map(l => l.id));
      } else {
  ...
  getNodeLayer(parentNodeId: number): ConceptNodeDTO[] {
    // Basic validation
    if (!this.userGraphData || this.userGraphData.id === -1 || !this.userGraphData.nodeMap || !this.userGraphData.nodeMap[parentNodeId]) {
  ...
  onTabClick(index: number): void {
    if (index >= 0 && index < this.openLayers.length -1) { // Prevent clicking the last (current) tab
        ...
        if(targetLayer && targetLayer.id !== this.currentLayer) {
            ...
            console.log("Navigated via Tab to Layer:", this.currentLayer, " Open Layers:", this.openLayers.map(l => l.id));
            this.changeActiveNode(this.currentNodeId).then(() => {
                this.router.navigate(['/dashboard/concept', this.currentNodeId]);
            });
        }
  ...
  onTableRowClick(node: ConceptNodeDTO, rowIndex: number, isRootLayer: boolean): void {
    console.log("Row Clicked:", { id: node.databaseId, name: node.name }, "Index:", rowIndex, "Is Root Layer:", isRootLayer);
    ...
    if (rowIndex === 0 && !isRootLayer) {
        if (this.openLayers.length > 1) { // Ensure there's a layer to go back to
