...
export class GraphStructureComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Constructor
  constructor(
    private readonly graphService: GraphTaskService,
  ...
  ngOnInit(): void {
    ...
    this.graphConfigurationSubscription = this.graphService
  ...
  calculateUI() {
    ...
    if (this.workspace && this.toolbar) {
      ...
      const {
        ...
        width: tWidth,
        height: tHeight,
      } = this.toolbar.nativeElement.getBoundingClientRect();
      this.toolbarRect = {
        // topLeft: { x: tX, y: tY },
        topLeft: { x: tX, y: tY },
  ...
  getReverseEdgeState(edge: IGraphEdge): 0 | 1 | 2 {
    ...
    if (!this.graphConfiguration.edgeDirected) {
      return 0;
    }

    const reverseEdge: IGraphEdge | undefined = this.edges.find(
      e => e.node1 === edge.node2 && e.node2 === edge.node1,
    );
