...
export function structuresAreEqual(
  ...
  for (let i = 0; i < structure1.nodes.length; i++) {
    const sameNode = structure2.nodes.find(
      node =>
  ...
  for (let i = 0; i < structure1Semantic.edges.length; i++) {
    const sameEdge = structure2Semantic.edges.find(
      edge =>
        edge.node1Value === structure1Semantic.edges[i].node1Value &&
        edge.node2Value === structure1Semantic.edges[i].node2Value,
    );
...
export function calculateLineCenter(
  from: PositionDTO,
  to: PositionDTO,
  size: SizeDTO,
  align: 0 | 1 | 2,
...
export function calculateEdgeStart(
  from: IGraphNode,
  to: IGraphNode,
  align: 0 | 1 | 2,
): PositionDTO {
