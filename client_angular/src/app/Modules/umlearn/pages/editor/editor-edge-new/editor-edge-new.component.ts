...
export class EditorEdgeNewComponent {
  ...
  @Output() edgeUpdated = new EventEmitter<{startDirection?: Side, startDirectionOffset?: number, endDirection?: Side, endDirectionOffset?: number}>();
  ...
  startElemProps: { x: number, y: number, width: number, height: number } = { x: 0, y: 0, width: 0, height: 0 };
  endElemProps: { x: number, y: number, width: number, height: number } = { x: 0, y: 0, width: 0, height: 0 };
  calculatedClosestSides: { startSide: Side, endSide: Side } = { startSide: Side.NONE, endSide: Side.NONE };
  highlightedElements: highlightedLineElements = {};

  private allInputsSet = false;
  private viewInitialized = false;

  constructor(private renderer: Renderer2, private edgeService: EditorEdgeService) {
    // handle edgeService events
    this.edgeService.movedNode.subscribe((nodeUuid: string) => {
      if (nodeUuid === this.edgeData?.start || nodeUuid === this.edgeData?.end || nodeUuid === 'all'){
        this.tryDrawLine();
      }
    });
    ...
    this.edgeService.reconnectedEdge.subscribe((edge: {id: string, start?: string | HTMLElement, end?: string | HTMLElement}) => {
  ...
  ngOnChanges(changes: SimpleChanges){
    if (changes['viewportElem'] && changes['startElem'] && changes['endElem'] && changes['viewportElem'].currentValue && changes['startElem'].currentValue && changes['endElem'].currentValue){

      this.allInputsSet = true;
      this.tryDrawLine();
    }
    if (changes['edgeData'] && changes['edgeData'].currentValue){
   ...
   */
  private tryDrawLine() {
    if (this.allInputsSet && this.viewInitialized && this.viewportElem && this.startElem && this.endElem) {
      this.position();
    }
  }
  ...
  private drawLine(): void {
    ...
    let direction: {start: number, end: number} = {start: 0, end: 0}
    const noPlugOffset = 0; // could be configurable
    const plugOffset: {start: number, end: number} = {start: (this.options.startPlug ? plugs[this.options.startPlug.plug as availablePlugs].lineOffset || noPlugOffset : noPlugOffset) * (this.options.startPlug ? this.options.startPlug.scale || 1 : 1), end: (this.options.endPlug ? plugs[this.options.endPlug.plug as availablePlugs].lineOffset || noPlugOffset : noPlugOffset) * (this.options.endPlug ? this.options.endPlug.scale || 1 : 1)};
    let line: SVGElement;

    // point offset
    if(this.options.startDirectionOffset){
      startPoint.x += (closestSides.startSide === Side.TOP || closestSides.startSide === Side.BOTTOM) ? this.options.startDirectionOffset : 0;
      startPoint.y += (closestSides.startSide === Side.LEFT || closestSides.startSide === Side.RIGHT) ? this.options.startDirectionOffset : 0;
    }
    if(this.options.endDirectionOffset){
      endPoint.x += (closestSides.endSide === Side.TOP || closestSides.endSide === Side.BOTTOM) ? this.options.endDirectionOffset : 0;
      endPoint.y += (closestSides.endSide === Side.LEFT || closestSides.endSide === Side.RIGHT) ? this.options.endDirectionOffset : 0;
    }
    let midLabelPos = {x: startPoint.x + (endPoint.x - startPoint.x) / 2, y: startPoint.y + (endPoint.y - startPoint.y) / 2};

    // lineType: direct
    if (!this.options.lineType || this.options.lineType === 'direct'){
      direction.start = Math.atan2(endPoint.y - startPoint.y, endPoint.x - startPoint.x) * 180 / Math.PI;
    ...
    // lineType: manhattan
      ...
      direction.start = closestSides.startSide === Side.RIGHT ? 0 : closestSides.startSide === Side.LEFT ? 180 : closestSides.startSide === Side.TOP ? 270 : 90;
      direction.end = closestSides.endSide === Side.RIGHT ? 0 : closestSides.endSide === Side.LEFT ? 180 : closestSides.endSide === Side.TOP ? 270 : 90;
    ...
    this.renderer.setAttribute(line, 'style', `stroke:${this.options.lineColor || 'black'}; stroke-width: ${this.options.size || '5'}; ${this.options.dash ? `stroke-dasharray: ${this.options.dash.len} ${this.options.dash.gap};`: ''} fill: none; stroke-linecap: ${(!this.options.lineType || this.options.lineType === 'direct') ? 'round' : 'round'};`);
    ...
    if (this.options.startLabel){
      startLabel = this.drawText(this.options.startLabel, startPoint, closestSides.startSide as Side);
    }
    ...
    if (this.options.endLabel){
      endLabel = this.drawText(this.options.endLabel, endPoint, closestSides.endSide as Side);
    }
    ...
    let lineGroup = this.renderer.createElement('g', 'svg');
    ...
    let labelGroup = this.renderer.createElement('g', 'svg');
    if (startLabel){
    ...
    if (this.edgeData && !this.edgeData.highlighted && !this.interactionsDisabled) {
      ...
      this.renderer.setAttribute(clickableLine, 'style', 'stroke: transparent; stroke-width: 30; fill: none');
   ...
   */
  private drawText(pTextData: TextLabel, pos: {x: number, y: number}, side: Side, highlight: boolean = false): SVGElement | undefined {
    const textData = {
      text: pTextData.text,
    ...
    const textX = ((side === Side.LEFT ? -textData.offset.x : side === Side.RIGHT ? textData.offset.x : 0) + pos.x).toString();
    const textY = ((side === Side.TOP ? -3*textData.offset.x : side === Side.BOTTOM ? 3*textData.offset.x : 0) + ((side === Side.LEFT || side === Side.RIGHT || side === Side.NONE) ? -textData.offset.y : 0) + pos.y).toString();
    ...
    (textStroke as any).textContent = textData.text;
    (textFill as any).textContent = textData.text;
   ...
   */
  private drawPlug(plugOptions: PlugOptions, pos: {x: number, y: number}, rotation: number): SVGElement | undefined {
    // check if plug is in the enum
    if (Object.values(availablePlugs).includes(plugOptions.plug as availablePlugs)){
      const plugKey: Plug = plugs[plugOptions.plug as availablePlugs];
      const plug = this.renderer.createElement('svg', 'svg') as SVGElement;
      this.renderer.setAttribute(plug, 'x', (pos.x - (plugKey.width * (plugOptions.scale || 1)) / 2).toString());
      this.renderer.setAttribute(plug, 'y', (pos.y - (plugKey.height * (plugOptions.scale || 1)) / 2).toString());
      this.renderer.setAttribute(plug, 'width', (plugKey.width * (plugOptions.scale || 1)).toString());
      this.renderer.setAttribute(plug, 'height', (plugKey.height * (plugOptions.scale || 1)).toString());
      this.renderer.setAttribute(plug, 'viewBox', plugKey.viewBox);

      if (plugKey.polygon){
        const polyGroup = this.renderer.createElement('g', 'svg');
        this.renderer.setAttribute(polyGroup, 'style', `transform-origin: center; transform: rotate(${rotation}deg);`); // rotate around the center of the plug'

        const polyStroke = this.renderer.createElement('polygon', 'svg');
        this.renderer.setAttribute(polyStroke, 'points', plugKey.polygon ?? '');
        this.renderer.setAttribute(polyStroke, 'style', `fill: ${!plugKey.scaleOrigin ? (plugOptions.outlineColor || plugOptions.color || 'black') : (plugOptions.outlineColor || plugOptions.color || 'black')};`);
        ...
        if (plugKey.scaleOrigin && plugOptions.outline){
          ...
          this.renderer.setAttribute(polyBase, 'style', `fill: ${plugOptions.color || 'white'}; transform-origin: ${plugKey.scaleOrigin}; transform: scale(${plugOptions.outline});`);
   ...
   */
  private setupDirect(startPoint: {x: number, y: number}, endPoint: {x: number, y: number}, lineOffset: {start: number, end: number} ): {line: SVGElement, textPoint: {x: number, y: number}} {

    const direction = Math.atan2(endPoint.y - startPoint.y, endPoint.x - startPoint.x) * 180 / Math.PI;
    // new start and end point by moving the points by the lineOffset
    startPoint = { x: startPoint.x + Math.cos(direction * Math.PI / 180) * lineOffset.start, y: startPoint.y + Math.sin(direction * Math.PI / 180) * lineOffset.start };
    endPoint = { x: endPoint.x - Math.cos(direction * Math.PI / 180) * lineOffset.end, y: endPoint.y - Math.sin(direction * Math.PI / 180) * lineOffset.end };
    const textPoint = {
   ...
   */
  private setupManhattan(pStartPoint: {x: number, y: number}, pEndPoint: {x: number, y: number}, closestSides: {startSide: Side, endSide: Side}, lineOffset: {start: number, end: number} ): {line: SVGElement, textPoint: {x: number, y: number}} {
    const startPoint = {...pStartPoint};
    const endPoint = {...pEndPoint};
    // new start and end point by moving the points by the lineOffset
    startPoint.x += closestSides.startSide === Side.RIGHT ? lineOffset.start : closestSides.startSide === Side.LEFT ? -lineOffset.start : 0;
    startPoint.y += closestSides.startSide === Side.TOP ? -lineOffset.start : closestSides.startSide === Side.BOTTOM ? lineOffset.start : 0;
    endPoint.x += closestSides.endSide === Side.RIGHT ? lineOffset.end : closestSides.endSide === Side.LEFT ? -lineOffset.end : 0;
    endPoint.y += closestSides.endSide === Side.TOP ? -lineOffset.end : closestSides.endSide === Side.BOTTOM ? lineOffset.end : 0;

    const line = this.renderer.createElement('path', 'svg');
    const isParallelSides: boolean = (closestSides.startSide === Side.RIGHT || closestSides.startSide === Side.LEFT) && (closestSides.endSide === Side.LEFT || closestSides.endSide === Side.RIGHT)
      || (closestSides.startSide === Side.TOP || closestSides.startSide === Side.BOTTOM) && (closestSides.endSide === Side.BOTTOM || closestSides.endSide === Side.TOP);
    ...
    const distanceStart = (lineOffset.start == 0 ? minimumDistance : lineOffset.start * 1.5) + additionalDistance; // TODO: make this configurable
    const distanceEnd = (lineOffset.end == 0 ? minimumDistance : lineOffset.end * 1.5) + additionalDistance; // TODO: make this configurable
    const pathStartPlugMarginPoint = closestSides.startSide === Side.RIGHT ? {
      x: startPoint.x + distanceStart, y: startPoint.y
    } : closestSides.startSide === Side.LEFT ? {
      x: startPoint.x - distanceStart, y: startPoint.y
    } : closestSides.startSide === Side.TOP ? {
      x: startPoint.x, y: startPoint.y - distanceStart
    } : {
      x: startPoint.x, y: startPoint.y + distanceStart
    };
    const pathEndPlugMarginPoint = closestSides.endSide === Side.RIGHT ? {
      x: endPoint.x + distanceEnd, y: endPoint.y
    } : closestSides.endSide === Side.LEFT ? {
      x: endPoint.x - distanceEnd, y: endPoint.y
    } : closestSides.endSide === Side.TOP ? {
      x: endPoint.x, y: endPoint.y - distanceEnd
    } : {
      x: endPoint.x, y: endPoint.y + distanceEnd
    };
    ...
    if (isParallelSides){
      if (closestSides.startSide === Side.RIGHT || closestSides.startSide === Side.LEFT){
        if (startDirection <= 180){
          ...
          midCornerPointStart = (endDirection <= 180) ? {
        ...
        if (endDirection <= 180){
          midCornerPointEnd = {
            ...
            x: pathEndPlugMarginPoint.x - (pathEndPlugMarginPoint.x - pathStartPlugMarginPoint.x) / 2,
            y: pathEndPlugMarginPoint.y
          };
        } else {
          //maybe later some 'around the element' logic, but then we need more mid points
          midCornerPointEnd = (startDirection <= 180) ? {
            x: pathEndPlugMarginPoint.x,
            y: pathStartPlugMarginPoint.y
          }: {
            // both directions 'outside'
            x: pathEndPlugMarginPoint.x,
            y: pathEndPlugMarginPoint.y - (pathEndPlugMarginPoint.y - pathStartPlugMarginPoint.y) / 2
          };
        }
        if (startDirection <= 180 || endDirection <= 180){
          textPoint = {
            x: (pathStartPlugMarginPoint.x + pathEndPlugMarginPoint.x) / 2,
            y: (midCornerPointStart.y < midCornerPointEnd.y) ? midCornerPointStart.y : midCornerPointEnd.y
          }
        }
      } else {
        if (startDirection <= 180){
    ...
    // if not parallel, most cases are 'edges'
      ...
      if (closestSides.startSide === Side.RIGHT || closestSides.startSide === Side.LEFT){
        if (startDirection <= 180){
          ...
          midCornerPointStart = endDirection <= 180 ? pathStartPlugMarginPoint : {
            x: pathStartPlugMarginPoint.x,
            y: pathEndPlugMarginPoint.y
          };
          midCornerPointEnd = endDirection <= 180 ? {
            x: pathStartPlugMarginPoint.x,
            y: pathEndPlugMarginPoint.y
          } : pathEndPlugMarginPoint;
          textPoint = endDirection <= 180 ? midCornerPointEnd: {
            x: (pathStartPlugMarginPoint.x + pathEndPlugMarginPoint.x) / 2,
            y: midCornerPointStart.y
          };
        }
  ...
  private calculateClosestSides(): { startSide: Side, endSide: Side } {
    ...
    if (this.options.startDirection){
      startSide = this.options.startDirection as Side || undefined;
    }
    if (this.options.endDirection){
      endSide = this.options.endDirection as Side || undefined;
    }
    ...
    startSides.forEach((start) => {
      endSides.forEach((end) => {
          const startMidX = start === Side.LEFT ? startRect.x :
              start === Side.RIGHT ? startRect.x + startRect.width : startRect.x + startRect.width / 2;

          const startMidY = start === Side.TOP ? startRect.y :
              start === Side.BOTTOM ? startRect.y + startRect.height : startRect.y + startRect.height / 2;

          const endMidX = end === Side.LEFT ? endRect.x :
              end === Side.RIGHT ? endRect.x + endRect.width : endRect.x + endRect.width / 2;

          const endMidY = end === Side.TOP ? endRect.y :
              end === Side.BOTTOM ? endRect.y + endRect.height : endRect.y + endRect.height / 2;
   ...
   */
  private getElemSideMidPoint(elem: 'startElement' | 'endElement', side: Side): { x: number, y: number } {
    const rect = elem === 'startElement' ? this.startElemProps : this.endElemProps;
    const x = side === Side.LEFT ? rect.x :
        side === Side.RIGHT ? rect.x + rect.width : rect.x + rect.width / 2;

    const y = side === Side.TOP ? rect.y :
        side === Side.BOTTOM ? rect.y + rect.height : rect.y + rect.height / 2;
  ...
  private setCustomOptions(): void {
    if (this.edgeData) {
      this.options = {...umlLineOptions[this.edgeData?.type as EditorElement || EditorElement.CD_ASSOCIATION].normal};
    }
    ...
    this.options.startLabel = {
      text: this.highlightedElements.missingLabel?.startLabel || this.edgeData?.cardinalityStart || '',
      size: 16,
      offset: {x: 10, y: 20},
      color: 'black',
      outline: 3,
  ...
  private handleHighlighting(): void {
    if (this.edgeData && this.edgeData.highlighted) {
      this.highlightedElements.line = (this.edgeData.highlighted.code == 'not_found' || this.edgeData.highlighted.updated?.type || this.edgeData.highlighted.updated?.start || this.edgeData.highlighted.updated?.start) ? true : false;
      this.highlightedElements.startLabel = (this.edgeData.highlighted.code == 'not_found' || this.edgeData.highlighted.updated?.cardinalityStart || this.edgeData.highlighted.added?.cardinalityStart) ? true : false;
      this.highlightedElements.middleLabel = (this.edgeData.highlighted.code == 'not_found' || this.edgeData.highlighted.updated?.description || this.edgeData.highlighted.added?.description) ? true : false;
      this.highlightedElements.endLabel = (this.edgeData.highlighted.code == 'not_found' || this.edgeData.highlighted.updated?.cardinalityEnd || this.edgeData.highlighted.added?.cardinalityEnd) ? true : false;
      this.highlightedElements.missing = this.edgeData.highlighted.code == 'missing' ? true : false;

      this.highlightedElements.missingLabel = {
        startLabel: this.edgeData.highlighted.deleted?.cardinalityStart ? this.edgeData.highlighted.deleted.cardinalityStart : '',
        middleLabel: this.edgeData.highlighted.deleted?.description ? this.edgeData.highlighted.deleted.description : '',
        endLabel: this.edgeData.highlighted.deleted?.cardinalityEnd ? this.edgeData.highlighted.deleted.cardinalityEnd : '',
      }
    }
  }
  ...
  position(): void {
    if (this.startElem && this.endElem && this.viewportElem){
      ...
      if (typeof this.startElem === 'string'){
        this.startElem = document.getElementById(this.startElem as string) || undefined;
      }
      // if end element is a string, try to get the element by id
      if (typeof this.endElem === 'string'){
        this.endElem = document.getElementById(this.endElem as string) || undefined;
      }
      if (!this.startElem || !this.endElem){
      ...
      this.startElemProps.x += this.startElem.style.transform ? parseInt(this.startElem.style.transform.split('(')[1].split('px')[0]) : 0;
      this.startElemProps.y += this.startElem.style.transform ? parseInt(this.startElem.style.transform.split(',')[1].split('px')[0]) : 0;
      this.endElemProps.x += this.endElem.style.transform ? parseInt(this.endElem.style.transform.split('(')[1].split('px')[0]) : 0;
      this.endElemProps.y += this.endElem.style.transform ? parseInt(this.endElem.style.transform.split(',')[1].split('px')[0]) : 0;
