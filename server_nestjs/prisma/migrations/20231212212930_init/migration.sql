...
CREATE TABLE `CodingQuestion` (
    ...
    `mainFileName` VARCHAR(191) NOT NULL,
    `questionId` INTEGER NOT NULL,
...
CREATE TABLE `KIFeedback` (
    ...
    `text` TEXT NOT NULL,
    `ratingByStudent` INTEGER NULL,
    `feedbackByStudent` TEXT NULL,
...
CREATE TABLE `Discussion` (
    ...
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
    `contentNodeId` INTEGER NULL,
    `conceptNodeId` INTEGER NOT NULL,
    `contentElementId` INTEGER NULL,
    `authorId` INTEGER NOT NULL,
    `isSolved` BOOLEAN NOT NULL DEFAULT false,

    UNIQUE INDEX `Discussion_authorId_key`(`authorId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `MCQuestion` ADD CONSTRAINT `MCQuestion_questionVersionId_fkey` FOREIGN KEY (`questionVersionId`) REFERENCES `QuestionVersion`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `UserMCOptionSelected` ADD CONSTRAINT `UserMCOptionSelected_userMCAnswerId_fkey` FOREIGN KEY (`userMCAnswerId`) REFERENCES `UserMCAnswer`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
