...
export class ChatBotController {
  ...
  async askBasicWithStreamAnswerDialog(
    ...
    if (context.length > 3) { // 3 is the minimum and default number including the default question by the LLM
      await this.chatBotRAGService.chatBotRagAnswerDialog(context, question, req.user.id, dialogSessionId);
    } else {
      await this.chatBotRAGService.chatBotRagAnswer(question, req.user.id, dialogSessionId);
    }
  }
  ...
  async askBasicWithAnswerDialog(
    ...
    // Choose the appropriate response generation method based on the context length
    if (context.length > 3) { // 3 is the minimum and default number including the default question by the LLM
      return await this.chatBotRAGService.chatBotRagAnswerDialog(context, question, req.user.id, dialogSessionId);
    } else {
      return await this.chatBotRAGService.chatBotRagAnswer(question, req.user.id, dialogSessionId);
    }
  }
