import type { freeTextQuestionDTO, freetextFeedbackRequestDTO } from '@Interfaces/index';
...
export class FeedbackGenerationService {
  constructor(
    ...
    private readonly llmService: LlmBasicPromptService,
  ...
  async generateFreetextFeedback(
    freeTextData: freeTextQuestionDTO,
    userAnswer: string,
  ): Promise<{ feedbackText: string; reachedPoints: number }> {
    console.log('FeedbackGenerationService.generateFreetextQuestion');

    const llmResponse = await this.llmService.generateLlmAnswer(
      feedbackGenerationPrompts.byExpectations(
        freeTextData.text,
        freeTextData.maxPoints,
        freeTextData.expectations,
        freeTextData.exampleSolution || undefined,
      ),
      userAnswer,
    );
    let reachedPoints = 0;
   ...
   */
  async generateFreetextTranscriptFeedback(
    requestData: freetextFeedbackRequestDTO,
  ): Promise<string> {
    console.log('FeedbackGenerationService.generateFreetextQuestion');
    ...
    const llmResponse = await this.llmService.generateLlmAnswer(
      feedbackGenerationPrompts.byTranscriptSearch(requestData.question, lecture_data),
      requestData.answer,
    );
  ...
  async getTranscripts(conceptNodeId: number): Promise<string> {
    ...
    const filteredTranscripts = transcripts.filter(transcript => {
      return this.prepareCompareString(transcript).includes(
        this.prepareCompareString(conceptNode.name),
      );
    });
    ...
    filteredTranscripts.forEach(transcript => {
      transcriptString +=
        '...' + fileSystem.readFileSync(path.join(transcriptsPath, transcript), 'utf8') + '...;\n';
    });
