...
export class UserConceptService {
  ...
  async updateUserLevel(userId: number, conceptNodeId: number, level: number): Promise<any> {
    ...
    if (userConcept === null) {
      console.log(
        `[UserConceptService] Creating UserConcept for user ${userId}, concept ${conceptNodeId} with level ${level}`,
      );
      ...
      if (userConcept.level < level) {
        console.log(
          `[UserConceptService] Updating UserConcept for user ${userId}, concept ${conceptNodeId} from ${userConcept.level} to ${level}`,
        );
        ...
        console.log(
          `[UserConceptService] No level update needed for user ${userId}, concept ${conceptNodeId}. Current: ${userConcept.level}, Proposed: ${level}`,
        );
      }
    }
    return userConcept;
  ...
  async checkUserConceptLevelAward(
    ...
    console.log(
      `[checkUserConceptLevelAward] Checking level award for user ${userId}, concept ${conceptNodeId}`,
    );
    let userConcept = await this.prisma.userConcept.findFirst({
      where: {
        conceptNodeId: conceptNodeId,
    ...
    const conceptNodeData = await this.prisma.conceptNode.findFirst({
      ...
      select: {
        ...
        trainedBy: {
          // Select the 'trainedBy' relation (Training records)
          select: {
            awards: true, // Select the 'awards' field from Training
            contentNode: {
              // Select the related 'contentNode'
              select: {
                id: true, // Select 'id' from ContentNode
                // title: true, // REMOVED: Invalid selection here
                ContentView: {
                  // Select the related 'ContentView'
                  select: {
                    contentElement: {
                      // Select the related 'contentElement'
                      select: {
    ...
    if (!conceptNodeData || !conceptNodeData.trainedBy) {
      console.error(
        `[checkUserConceptLevelAward] ConceptNode with ID ${conceptNodeId} not found or has no training links.`,
      );
      return [false, userConcept.level]; // Return current state if no training links
    }
    console.log(
      `[checkUserConceptLevelAward] ConceptNode: ${conceptNodeData.name} (ID: ${conceptNodeId})`,
    );
    ...
    for (const trainingRecord of conceptNodeData.trainedBy) {
      ...
      console.log(
        `[checkUserConceptLevelAward] -- Checking Training Record: Awards Level ${trainingRecord.awards}, ContentNode ID: ${trainingRecord.contentNode.id}`,
      );
      ...
      if (trainingRecord.contentNode.ContentView) {
        ...
        for (const contentView of trainingRecord.contentNode.ContentView) {
          ...
          if (contentView.contentElement) {
            ...
            console.log(
              `[checkUserConceptLevelAward] ---- Checking ContentElement ID: ${contentElement.id} ('${contentElement.title}'), Type: ${contentElement.type}`,
            );
            ...
            if (contentElement.type === 'QUESTION') {
              ...
              console.log(
                `[checkUserConceptLevelAward] ------ User Progress for Question ${
                  contentElement.id
                }: ${userProgress ? `markedAsDone=${userProgress.markedAsDone}` : 'Not found'}`,
              );
              if (!userProgress || !userProgress.markedAsDone) {
                allQuestionsCompleted = false;
                console.log(
                  `[checkUserConceptLevelAward] ------ Question ${contentElement.id} is NOT done. Breaking check for this ContentNode.`,
                );
            ...
            console.log(
              `[checkUserConceptLevelAward] ---- Skipping contentView as contentElement is missing.`,
            );
          }
        }
        ...
        if (questionCount === 0) {
          console.log(
            `[checkUserConceptLevelAward] -- No QUESTION elements found in ContentNode ID: ${trainingRecord.contentNode.id}. Cannot be completed.`,
          );
          allQuestionsCompleted = false;
        }
      } else {
        ...
        console.log(
          `[checkUserConceptLevelAward] -- ContentNode or ContentView missing for Training Record awarding level ${trainingRecord.awards}. Cannot be completed.`,
        );
        allQuestionsCompleted = false;
      }

      console.log(
        `[checkUserConceptLevelAward] -- Result for ContentNode ID ${trainingRecord.contentNode.id}: allQuestionsCompleted = ${allQuestionsCompleted}`,
      );
      // If this ContentNode is fully completed, consider its awarded level
      if (allQuestionsCompleted && trainingRecord.awards !== null) {
        console.log(
          `[checkUserConceptLevelAward] -- ContentNode ID ${trainingRecord.contentNode.id} IS complete. Considering its award level: ${trainingRecord.awards}`,
        );
        anyContentNodeCompleted = true;
        highestLevelAchieved = Math.max(highestLevelAchieved, trainingRecord.awards);
        console.log(
          `[checkUserConceptLevelAward] -- Current highestLevelAchieved: ${highestLevelAchieved}`,
        );
      }
    }

    console.log(
      `[checkUserConceptLevelAward] Final check: anyContentNodeCompleted=${anyContentNodeCompleted}, highestLevelAchieved=${highestLevelAchieved}, current user level=${userConcept.level}`,
    );
    ...
    if (anyContentNodeCompleted && highestLevelAchieved > userConcept.level) {
      console.log(
        `[checkUserConceptLevelAward] Condition met for update. Calling updateUserLevel...`,
      );
      // Use the existing updateUserLevel function which handles event creation
      const updatedUserConcept = await this.updateUserLevel(
        userId,
        conceptNodeId,
    ...
    console.log(
      `[checkUserConceptLevelAward] Returning: [levelUpdated=${levelUpdated}, finalLevel=${userConcept.level}]`,
    );
  ...
  async updateUserConceptExpansionState(
    ...
    // if the userConcept exists, update it
      ...
      if (userConcept.expanded !== expanded) {
