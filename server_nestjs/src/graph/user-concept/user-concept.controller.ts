...
export class UserConceptController {
  constructor(private readonly userConceptService: UserConceptService) {}

  @roles('ANY')
  @Put(':conceptId/level/:level')
  async updateUserConceptLevel(
    @Req() req,
  ...
  async updateUserConceptExpanded(
    @Req() req,
    @Param('conceptId', ParseIntPipe) conceptId: number,
    @Param('expanded') expandedString: string,
  ): Promise<any> {
    const userId = req.user.id;
    const expandedBoolean = expandedString === 'true';

    return await this.userConceptService.updateUserConceptExpansionState(
      userId,
      conceptId,
      expandedBoolean,
    );
  ...
  async updateUserConceptSelected(
    @Req() req,
    @Param('conceptId', ParseIntPipe) conceptId: number,
  ): Promise<any> {
    const userId = req.user.id;
