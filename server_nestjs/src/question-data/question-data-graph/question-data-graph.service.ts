...
export class QuestionDataGraphService {
  ...
  async getGraphQuestion(questionId: number, fullData = false): Promise<GraphQuestionDTO> {
    ...
    return {
      ...
      exampleSolution: fullData
        ? JSON.parse(JSON.stringify(graphQuestion.exampleSolution)) || undefined
        : undefined,
  ...
  async createGraphQuestion(
    graphQuestion: detailedGraphQuestionDTO,
    questionId: number,
  ): Promise<detailedGraphQuestionDTO> {
  ...
  async updateGraphQuestion(
    ...
    const updatedGraphQuestion = await this.prisma.graphQuestion.update({
      ...
      data: {
        ...
        initialStructure:
          JSON.parse(JSON.stringify(graphQuestion.initialStructure)) ||
          originalGraphQuestion.initialStructure,
        exampleSolution:
          JSON.parse(JSON.stringify(graphQuestion.exampleSolution)) ||
          originalGraphQuestion.exampleSolution,
        stepsEnabled: graphQuestion.stepsEnabled || originalGraphQuestion.stepsEnabled,
        configuration:
          JSON.parse(JSON.stringify(graphQuestion.configuration)) ||
          originalGraphQuestion.configuration,
      },
    });
