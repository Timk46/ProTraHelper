...
export class QuestionDataService {
  ...
  async createUserAnswer(
    ...
    if (question.type === questionType.MULTIPLECHOICE) {
      ...
      console.log(`[MC Evaluation] Using questionScore: ${questionScore} (original: ${question.score})`);
      ...
      console.log(`[MC Evaluation] Total options: ${mcOptions.length}, Correct options: ${correctOptions.length}, Is Single Choice: ${isSingleChoice}`);
      
      if (isSingleChoice) {
        // Single Choice: Full points if the one correct option is selected
        const correctOptionId = correctOptions[0].id;
        if (answerData.userMCAnswer.includes(correctOptionId) && answerData.userMCAnswer.length === 1) {
          userScore = questionScore; // Full points
          console.log(`[MC Evaluation] Single Choice correct: awarded full ${questionScore} points`);
        } else {
        ...
        console.log(`[MC Evaluation] Multiple Choice: calculated ${userScore} points from ${mcOptions.length} options`);
      }
    ...
    if (question.type === questionType.SINGLECHOICE) {
      ...
      const questionScore = question.score || 3;
      console.log(`[SC Evaluation] Using questionScore: ${questionScore} (original: ${question.score})`);
    ...
    if (question.type === questionType.MCSLIDER) {
      ...
      // Ensure consistent score handling with nullish coalescing
      const questionScore = question.score ?? 3;
      console.log(`[MC-Slider Debug] Using questionScore: ${questionScore} (original: ${question.score})`);
      ...
      console.log(`[MC-Slider Debug] Options data:`, mcOptions.map(opt => ({ id: opt.id, text: opt.text, correct: opt.correct })));
      ...
      console.log(`[MC-Slider Debug] Total options: ${mcOptions.length}, Correct options: ${correctOptions.length}, Is Single Choice: ${isSingleChoice}`);
      ...
      if (isSingleChoice) {
        ...
        if (answerData.userMCAnswer.includes(correctOptionId) && answerData.userMCAnswer.length === 1) {
          ...
          console.log(`[MC-Slider Debug] Single Choice correct: awarded full ${questionScore} points`);
        } else {
          userScore = 0; // No points for wrong single choice
          progress = 0;
        ...
        for (const mcOption of mcOptions) {
          if (mcOption.correct && answerData.userMCAnswer.includes(mcOption.id)) {
            ...
            console.log(`[MC-Slider Debug] ✓ Correctly selected: ${mcOption.text} (ID: ${mcOption.id})`);
          } else if (!mcOption.correct && !answerData.userMCAnswer.includes(mcOption.id)) {
            correctAnswers++; // Correctly did NOT select an incorrect option
            console.log(`[MC-Slider Debug] ✓ Correctly NOT selected: ${mcOption.text} (ID: ${mcOption.id})`);
          } else if (mcOption.correct && !answerData.userMCAnswer.includes(mcOption.id)) {
            console.log(`[MC-Slider Debug] ✗ Missed correct option: ${mcOption.text} (ID: ${mcOption.id})`);
          } else if (!mcOption.correct && answerData.userMCAnswer.includes(mcOption.id)) {
            console.log(`[MC-Slider Debug] ✗ Incorrectly selected: ${mcOption.text} (ID: ${mcOption.id})`);
          }
        }
        
        console.log(`[MC-Slider Debug] Final score calculation: ${correctAnswers}/${totalOptions} = ${correctAnswers/totalOptions * 100}%`);
        ...
        if (scorePercentage === 1) {
          feedbackText = `Richtig beantwortet! Du hast alle ${totalOptions} Optionen korrekt bewertet (${Math.round(scorePercentage * 100)}%). Punktzahl: ${userScore} von ${questionScore} Punkten`;
        } else {
          feedbackText = `Du hast ${correctAnswers} von ${totalOptions} Optionen korrekt bewertet (${Math.round(scorePercentage * 100)}%). Punktzahl: ${userScore} von ${questionScore} Punkten`;
        }
      }
      ...
      console.log(`MC-Slider feedback: ${feedbackText}, score: ${userScore}, progress: ${progress}`);
    ...
    if (question.type === questionType.CODEGAME) {
      ...
      // Ensure consistent score handling with fallback
      const questionScore = question.score || 3;
      console.log(`[CodeGame Evaluation] Using questionScore: ${questionScore} (original: ${question.score})`);
