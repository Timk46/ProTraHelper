...
export class RhinoDirectService {
  ...
  async getSystemRhinoInfo(): Promise<SystemRhinoInfo> {
    ...
    try {
      ...
      if (process.platform === 'win32') {
        ...
        if (registryInstallations.length > 0) {
          defaultPath = registryInstallations.find(r => r.isDefault)?.path || registryInstallations[0].path;
        }
      }
    } catch (error) {
    ...
    console.log('✅ System Rhino info gathered:', { installations: installations.length, defaultPath });
  ...
  private async executeRhinoProcess(rhinoPath: string, command: string): Promise<number> {
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (!process.pid) {
          reject(new Error('Rhino-Prozess konnte nicht innerhalb von 10 Sekunden gestartet werden'));
        }
      }, 10000);
    });
  }
