 ...
 */
const ktcSystemPrompt = `You are the Knowledge of Test Cases (KTC) agent.
...
export function createKtcAgent(llm: ChatOpenAI) { // Accept only llm
    // Use the common helper function to create the agent, passing an empty tools array
    return createFeedbackAgent('KTC', ktcSystemPrompt, llm, []);
}
