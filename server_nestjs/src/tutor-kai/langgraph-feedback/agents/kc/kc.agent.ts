...
import { getConceptsPrompt, generateFeedbackPrompt } from './kc.prompts';
...
const identifyConceptsAndRequestTool = async (
  ...
  if (!context) {
    return {
      messages: [...state.messages, new AIMessage('Error: Could not parse input context.')],
    };
  }
  ...
  const messagesA = await getConceptsPrompt.formatMessages({
    ...
    unitTests: context.automatedTests
      ? JSON.stringify(context.automatedTests)
      : 'Es liegen keine Unit-Tests vor.',
    unitTestsResults: context.unitTestResults
      ? JSON.stringify(context.unitTestResults)
 ...
 */
const processToolResults = async (state: KcGraphState): Promise<Partial<KcGraphState>> => {
  ...
  try {
    ...
    if (typeof lastMessage.content === 'string') {
      try {
        ...
        console.warn(
          'ToolMessage content is a string but not valid JSON. Treating as empty result.',
          parseError,
        );
    ...
    if (!Array.isArray(parsedContent)) {
      ...
      console.error(
        `Processed tool output is not an array. Type: ${typeof parsedContent}. Content:`,
        parsedContent,
      );
    ...
    if (parsedContent.length > 0) {
      ...
      if (
        ...
        typeof firstElement.metadata !== 'object' ||
        firstElement.metadata === null ||
        typeof firstElement.metadata.markdownLink !== 'string'
      ) {
        console.error(
          'Parsed array elements do not match TranscriptChunk structure:',
          firstElement,
        );
...
const generateFeedback = async (
  ...
  if (!context) {
    ...
    return {
      messages: [
        ...state.messages,
        new AIMessage('Error: Input context missing for final feedback generation.'),
      ],
    };
  }
 ...
 */
const processFeedbackCitations = async (state: KcGraphState): Promise<Partial<KcGraphState>> => {
  ...
  if (!(lastMessage instanceof AIMessage) || typeof lastMessage.content !== 'string') {
    ...
    const fallbackContent =
      ...
      'Error: Could not process feedback citations.';
 ...
 */
export function buildKcCoreAgent(
  llm: ChatOpenAI,
  tools: DynamicStructuredTool[],
): Runnable<any, any> {
  console.log(
    'Building KC Core Agent with tools:',
    tools.map(t => t.name),
  );
  ...
  if (!domainKnowledgeTool) {
    ...
    throw new Error(
      "buildKcCoreAgent requires 'search_domain_knowledge' to be provided in the tools array.",
  ...
  const agentRunnable = new RunnableLambda({
    ...
    func: async (
      input: { messages: BaseMessage[]; context: FeedbackContextDto },
      config?: RunnableConfig,
    ) => {
      console.log('--- KC Agent Runnable Wrapper: Invoked ---');
      // Validate both messages and context in the input
      if (!input || !input.context || !Array.isArray(input.messages)) {
        // Check context existence
        console.error(
          'KC Agent Wrapper: Invalid input format (missing messages or context):',
          input,
        );
      ...
      try {
        ...
        const finalMessageContent =
          finalState.finalFeedback ?? 'Error: No feedback generated or processed.';
        ...
        return {
          messages: [
            new AIMessage({ content: finalState.finalFeedback ?? 'Error: No feedback generated.' }),
          ],
        };
      } catch (error) {
