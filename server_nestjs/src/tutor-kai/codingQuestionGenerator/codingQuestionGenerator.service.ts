  ...
  async genTask(topic: string, context: string): Promise<genTaskDto> {
    ...
    const genTask = async (state) => {
        ...
        const prompt = ChatPromptTemplate.fromMessages([
            ...
            ""`),
            new HumanMessage("Erstelle eine Programmieraufgabe für die Programmiersprache Python zu dem Programmierkonzept \"" + paramTopic + "\". Als Kontext soll das Themengebiet \"" + paramContext + "\" dienen. Das Themengebiet soll sinnvoll in die Aufgabe eingebettet werden. Die Aufgabe soll keine Eingaben über die Standardeingabe erwarten. Ausgaben über die Konsole sind möglich aber nicht verpflichtend. Gib keine Lösung in Form von Code an. Gib keine Hinweise in der Aufgabe."),
        ])

        model.temperature = 0.2;
        const response = await prompt.pipe(new ChatOpenAI({ temperature: 0, streaming: true, model: gptModel })).invoke({ messages });
        model.temperature = 0;
    ...
    const genSolution = async (state) => {
        ...
        if (state.checkCodeErrorHappend) {
            prompt = ChatPromptTemplate.fromMessages([
                new SystemMessage(`Du bist ein Programmierexperte. Gib immer nur den ausführbaren Code an. Ergänze niemals normalen Text in der Ausgabe.`),
                ...
                Die aufgetauchte Fehlermeldung: ${state.checkCodeError[state.checkCodeError.length - 1]}`),
    ...
    const genCodeFramework = async (state) => {
        const { messages } = state;
    ...
    const genUnitTest = async (state) => {
        ...
        if (state.checkCodeErrorHappend) {
            ...
            prompt = ChatPromptTemplate.fromMessages([
                ...
                if __name__ == '__main__':
                        run_tests()`),
                new HumanMessage("Entwickle einen Unit-Test für eine Programmieraufgabe inklusive aller benötigten Imports zu der nachfolgenden Musterlösung. Generiere nur den Unit-Test ohne zusätzlichen Text. Der Unit-Test soll direkt ausführbar sein."),
                new MessagesPlaceholder("messages"),
            ]);
        }
    ...
    const genExpectation = async (state) => {
        const { messages } = state;
    ...
    const checkCode = async (state) => {
        ...
        if (!testsPassed) {
            ...
            for (let i = 0; i < testResults.length; i++) {
                if (testResults[i].status === "FAILED") { //Es gab einen Fehler in einem Testcase
                ...
                } else if (testResults[i].status === "ERROR") { //ERROR -> alter Kompilierungsfehler
                    const testCase = testResults[i].test;
                    const testError = testResults[i].error;

                    errorMessages += `Fehler: ${testCase}\n Fehlermeldung: ${testError} \n`;
                } else if (testResults[i].status === "MAIN_COMPILATION") { //Kompilierungsfehler (neu seid Jury1 update)
    ...
    const _CheckCodePassed = (state): string => {
        ...
        if (state.checkCodeErrorHappend === true && state.checkCodeIteration < maxIterations) {
            console.log("----------------------- SOLUTION ERROR => Retry! -----------------------", state.checkCodeIteration);
            return "retry";
        } else {
            return "continue";
        }
    ...
    const genRunMethodArgs = async (state) => {
        const { messages } = state;

        const prompt = ChatPromptTemplate.fromMessages([
            new SystemMessage("Du bist ein JSON-Experte. Gib immer nur das direkte JSON-Objekt wieder, welches sofort genutzt werden kann."),
        ...
        try {
            ...
            console.log("Die Methode lautet: " + jsonData.runMethod + " und die Argumente sind: " + jsonData.input);
        } catch (error) {
    ...
    const judgeGenTask = async (state) => {
        const { messages } = state;

        const prompt = ChatPromptTemplate.fromMessages([
            new SystemMessage(`Du bist ein Dozent an einer Hochschule und leitest den Kurs 'Einführung in die Informatik'. Im Rahmen dieses Kurses sollst du Programmieraufgaben bewerten. Die Programmieraufgaben wurden von einer künstlichen Intelligenz erstellt und sollen nun von dir bewertet werden`),
    ...
    const workflow = new StateGraph({
        channels: taskGenState,
    });

    workflow.addNode("genTask", genTask);
    workflow.addNode("genSolution", genSolution);
    workflow.addNode("genUnitTest", genUnitTest);
    workflow.addNode("genExpectation", genExpectation);
    workflow.addNode("checkCode", checkCode);
    workflow.addNode("genCodeFramework", genCodeFramework);
    workflow.addNode("genRunMethodArgs", genRunMethodArgs);
    workflow.addNode("judgeGenTask", judgeGenTask);

    // Neue Workflow Test: GenTask |-> GenSolution -> genExpectation -> GenUnitTest -> CheckCode|
    workflow.setEntryPoint("genTask");
    workflow.addEdge("genTask", "genCodeFramework");
    workflow.addEdge("genCodeFramework", "genSolution");
    // workflow.addEdge("genSolution", "genExpectation");
    // workflow.addEdge("genExpectation", "genUnitTest");
    workflow.addEdge("genSolution", "genUnitTest");
    workflow.addEdge("genUnitTest", "checkCode");

    workflow.addConditionalEdges("checkCode", _CheckCodePassed, {
        retry: "genSolution",
        continue: "genExpectation",
    });

    workflow.addEdge("genExpectation", "genRunMethodArgs");
    //workflow.addEdge("genRunMethodArgs", END);
    workflow.addEdge("genRunMethodArgs", "judgeGenTask");
    workflow.addEdge("judgeGenTask", END);
    ...
    const result = await app.invoke({}, { recursionLimit: langGraphRecursionLimit });
    ...
    console.log(`Nach dem ${result.checkCodeIteration}. Versuche den Code zu testen (Anzahl Solutions: ${result.solution.length}, Anzahl UnitTests: ${result.unitTest.length}) lag noch ein Fehler vor (true = ja / false = nein): ${result.checkCodeErrorHappend}`);
    ...
    return {id: dbEntry.id, task: result.task, expectation: result.expectation, solution: result.solution[result.solution.length - 1], unitTest: result.unitTest[result.unitTest.length - 1], codeFramework: result.codeFramework, hasFailure: result.checkCodeErrorHappend, iterations: result.checkCodeIteration, runMethod: result.runMethod, runMethodInput: result.runMethodInput};
...
//////////////////////////////////////////////////////////////////////////////////////////////////////// OLD CODE ////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     * @returns a JSON object with the task, expectation, solution, unitTest, codeFramework and hasFailure (true = error in Response from Jury1 during testing Solution and Unittest, false = No error from Jury1)
     */
    async genTaskByTopic(topic: string, context: string): Promise<genTaskDto> {
