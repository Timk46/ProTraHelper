...
export class AppController {
  ...
  getStudent(@Req() req): string { // Aways access user data like this. This is validated by the jwt strategy!
  ...
  getAdminteacher(@Req() req): string {
    return "Route worked for admin or teacher. UserId:"  + req.user.id + " and email " + req.user.email + " and role " + req.user.globalRole
  }
