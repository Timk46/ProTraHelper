...
export class TransitiveClosureService {
  ...
  evaluateSolution(
    ...
    const { receivedPoints, feedbackHTML, expectedSolutionSemantic } = this.algorithmicEvaluation(
      initialStructureSemantic,
      studentSolutionSemantic,
      maxPoints,
    );
  ...
  algorithmicEvaluation(
    initialStructureSemantic: GraphStructureSemanticDTO,
    studentSolutionSemantic: GraphStructureSemanticDTO,
    maxPoints: number,
  ) {
    ...
    const expectedSolutionSemantic: GraphStructureSemanticDTO = this.solveTransitiveClosure(
      initialStructureSemantic,
      nodesOrder,
    );

    // Check if the studentSolution contains all required nodes and no unnecesary nodes
    const sameNodesExist = graphsContainSameNodes(
      studentSolutionSemantic,
    ...
    const studentMissingInitialEdges = getExtraEdges(
      initialStructureSemantic.edges,
      studentSolutionSemantic.edges,
    );
    ...
    const edgesToAdd: GraphEdgeSemanticDTO[] = getExtraEdges(
      expectedSolutionSemantic.edges,
      initialStructureSemantic.edges,
    );
    const studentNewEdges: GraphEdgeSemanticDTO[] = getExtraEdges(
      studentSolutionSemantic.edges,
      initialStructureSemantic.edges,
    );
    ...
    receivedPoints =
      maxPoints -
      maxPoints * ((countStudentExtraEdges + countStudentMissingEdges) / countEdgesToAdd);
