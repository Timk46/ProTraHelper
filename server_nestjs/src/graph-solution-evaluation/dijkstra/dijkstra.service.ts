...
import {
  getExtraSelectedNodes,
  getNodesWithDifferentWeights,
  graphJSONToSemantic,
...
export class DijkstraService {
  ...
  evaluateSolution(
    ...
    for (let stepIndex = 0; stepIndex < studentSolutionSemantic.length; stepIndex++) {
      ...
      if (stepIndex >= initialStructure.nodes.length) {
        feedback.push(
          `Die Anzahl der Lösungsschritte überschreitet der maximalen erwarteten Schritte.`,
        );
      ...
      const studentPrevStep: GraphStructureSemanticDTO =
        stepIndex > 0
      ...
      if (!sameNodesExist || !sameEdgesExist) {
        ...
        feedback.push(
          `> Punkte für diesen Schritt: ${stepReceivedPoints.toFixed(2)} / ${maxStepPoints.toFixed(
            2,
          )}`,
        );
        feedback.push('\n###############\n');
      ...
      const studentNewVisitedNodes: GraphNodeSemanticDTO[] = getExtraSelectedNodes(
        studentStep.nodes,
        studentPrevStep.nodes,
      );
      ...
      const expectedPrevStep: GraphStructureSemanticDTO =
        stepIndex > 0
      ...
      const countDifferentWeights: number = getNodesWithDifferentWeights(
        studentStep.nodes,
        expectedStep.nodes,
      ).length;
      let tempPoints: number =
        maxStepPoints / 2 - (maxStepPoints / 2) * (countDifferentWeights / countNodes);
      tempPoints = Math.max(tempPoints, 0);
      stepReceivedPoints += tempPoints;

      // Feedback for weight differences
      const nodesWithDifferentWeights: string[] = getNodesWithDifferentWeights(
        studentStep.nodes,
        expectedStep.nodes,
      );
    ...
    if (
      evaluateNextSteps === false &&
      lastEvaluatedStepIndex < studentSolutionSemantic.length - 1
    ) {
  ...
  findPossibleNextStep(
    previousStep: GraphStructureSemanticDTO,
    visitedNodeValue: string,
  ): {
    ...
    previousStep.edges.forEach(edge => {
      ...
      if (neighborNodePrev.weight > visitedNodePrev.weight + edge.weight) {
        nextStep.nodes.find(node => node.value === neighborNodeValue).weight =
          visitedNodePrev.weight + edge.weight;
      }
    });
