...
export class KruskalService {
  evaluateSolution(
    ...
    const studentNewEdgesByStep: GraphEdgeSemanticDTO[][] = [];
    const studentRemovedEdgesByStep: GraphEdgeSemanticDTO[][] = [];
    ...
    for (let stepIndex = 0; stepIndex < studentSolutionSemantic.length; stepIndex++) {
      ...
      const prevStepEdges: GraphEdgeSemanticDTO[] =
        stepIndex > 0 ? studentSolutionSemantic[stepIndex - 1].edges : [];
    ...
    const allPossibleSolutions: GraphEdgeSemanticDTO[][] = this.generatePossibleMSTs(
      initialStructureSemantic.edges,
    );
    ...
    allPossibleSolutions.forEach((possibleSolution: GraphEdgeSemanticDTO[]) => {
      ...
      const { possibleReceivedPoints, possibleFeedback, possibleFeedbackHTML } =
        this.evaluatePossibleSolution(
          studentNewEdgesByStep,
          studentRemovedEdgesByStep,
          possibleSolution,
          maxPoints,
        );
  ...
  evaluatePossibleSolution(
    studentNewEdgesByStep: GraphEdgeSemanticDTO[][],
    studentRemovedEdgesByStep: GraphEdgeSemanticDTO[][],
    possibleSolution: GraphEdgeSemanticDTO[],
    maxPoints: number,
  ) {
    ...
    for (let stepIndex = 0; stepIndex < studentNewEdgesByStep.length; stepIndex++) {
      ...
      if (stepIndex >= possibleSolution.length) {
        ...
        feedbackHTML +=
          'Die Anzahl der Lösungsschritte überschreitet maximalen erwarteten Schritte.\n';
        feedbackHTML += `\n<strong>> Insgesamt erzielte Punkte:</strong> ${receivedPoints.toFixed(
          2,
        )} / ${maxPoints}.`;
        return {
          feedback,
      ...
      if (studentRemovedEdges.length > 0) {
        ...
        feedback += '\n###############\n\n';
        feedbackHTML +=
          'Hinzugefügte Kanten können in nächsten Schritten nicht gelöscht werden.\n\n';
        violation = true;
        break;
      }
  ...
  findRoot(unionFind: IUnionFind, element: string): string {
    while (unionFind.parent[element] !== element) {
  ...
  kruskalRecursive(
    ...
    findOne = false, // Flag to stop after finding one MST
  ): GraphEdgeSemanticDTO[][] {
    ...
    let minWeightEdges: {
      candidateEdge: GraphEdgeSemanticDTO; // A candidate edge to add to the MST
      otherEdges: GraphEdgeSemanticDTO[]; // Remaining edges after removing this candidate
    }[] = [];
    ...
    for (const edgeGroup of minWeightEdges) {
      ...
      if (
        !this.areConnected(
          clonedUnionFind,
          edgeGroup.candidateEdge.node1Value,
          edgeGroup.candidateEdge.node2Value,
        )
      ) {
        this.unionSets(
          clonedUnionFind,
      ...
      const newMSTs = this.kruskalRecursive(
        edgeGroup.otherEdges,
        clonedUnionFind,
        clonedMST,
        findOne,
      );
  ...
  generatePossibleMSTs(edges: GraphEdgeSemanticDTO[], findOne = false): GraphEdgeSemanticDTO[][] {
