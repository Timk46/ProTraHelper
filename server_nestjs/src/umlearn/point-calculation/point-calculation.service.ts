...
export class PointCalculationService {
  private readonly pointDefinitions = pointDefinitions;
   ...
   * @returns
   */
  async defineNodePoints(
    node: ClassNode,
  ): Promise<{ attributePoints: number; methodPoints: number }> {
    return {
  ...
  async defineEdgePoints(edge: ClassEdge): Promise<number> {
    return (
      this.pointDefinitions.edge.found +
  ...
  async calculatePoints(
    highlightData: editorDataDTO,
    solutionData: editorDataDTO,
    maxPoints = -1,
  ): Promise<number> {
    let points = 0;
    ...
    // calculate points for nodes
    for (const node of highlightData.nodes) {
      if (node.highlighted && node.highlighted.code == 'found' && node.highlighted.maxPoints) {
        ...
        solutionPoints +=
          this.pointDefinitions.node.found +
        ...
        for (const attr of node.attributes) {
          ...
          for (const hAttr of node.highlighted.updated.attributes) {
            if (hAttr && hAttr.name == attr.name) {
              attrPoint = 0;
            }
          }
          if (node.highlighted.deleted.attributes) {
            for (const hAttr of node.highlighted.deleted.attributes) {
              if (hAttr && hAttr.name == attr.name) {
                attrPoint = 0;
              }
            }
        ...
        let addedPoints = 0;
        for (const hAttr of node.highlighted.added.attributes) {
          if (hAttr && hAttr != null) {
            addedPoints++;
          }
        }
        ...
        for (const method of node.methods) {
          ...
          for (const hMethod of node.highlighted.updated.methods) {
            if (hMethod && hMethod.name == method.name) {
              methodPoint = 0;
            }
          }
          if (node.highlighted.deleted.methods) {
            for (const hMethod of node.highlighted.deleted.methods) {
              if (hMethod && hMethod.name == method.name) {
                methodPoint = 0;
              }
            }
        ...
        addedPoints = 0;
        for (const hMethod of node.highlighted.added.methods) {
    ...
    // calculate points for edges
    for (const edge of highlightData.edges) {
      if (edge.highlighted && edge.highlighted.code == 'found' && edge.highlighted.maxPoints) {
        ...
        for (const key in edge.highlighted) {
    ...
    console.log(
      ...
      solutionPoints,
      'Percentage: ',
      Math.floor((points / possiblePoints) * maxPoints),
      possiblePoints,
    );
