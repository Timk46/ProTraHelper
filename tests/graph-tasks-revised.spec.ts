  ...
  test('should complete transitive closure task ID 29 successfully', async ({ page }) => {
    ...
    try {
      ...
      const isRegisteredForAlgorithms = subjects.some(
        (s: any) => s.subjectname === 'Algorithmen und Datenstrukturen' && s.registeredForSL === true
      );
      
      const isRegisteredForOOP = subjects.some(
        (s: any) => s.subjectname === 'Objektorientierte und funktionale Programmierung' && s.registeredForSL === true
      );
      
      // If registered for OOP but not for Algorithms, skip the actual test
      if (isRegisteredForOOP && !isRegisteredForAlgorithms) {
        console.log('User is registered for "Objektorientierte und funktionale Programmierung" but not for "Algorithmen und Datenstrukturen". Skipping test.');
        return; // Skip the test and mark as passed
      }
      
      // If not registered for Algorithms, fail the test
      if (!isRegisteredForAlgorithms) {
        throw new Error('User is not registered for "Algorithmen und Datenstrukturen". Test cannot proceed.');
      }
      
      console.log('User is registered for "Algorithmen und Datenstrukturen". Proceeding with test.');
      ...
      while (retryCount < maxRetries) {
        try {
          await page.goto('http://localhost:4200/graphtask/29', { timeout: 30000 });
          
          // Wait for the graph to be fully loaded
          await page.waitForSelector('.workspace-boundary', { state: 'visible', timeout: 15000 });
          ...
          if (retryCount >= maxRetries) {
            throw new Error(`Failed to navigate to task page after ${maxRetries} attempts`);
          }
      ...
      await page.waitForSelector('.node-div', { state: 'visible', timeout: 15000 });
      ...
      await page.waitForSelector('.evaluation', { state: 'visible', timeout: 20000 });
      ...
      try {
        ...
        const visibleNodes = await page.locator('.node-div .node-value').allTextContents();
...
async function addDirectedEdge(page: Page, sourceNodeValue: string, targetNodeValue: string): Promise<void> {
  try {
    ...
    const isContainerVisible = await toolsetContainer.isVisible().catch(() => false);
    ...
    try {
      ...
      console.log('Failed to click outgoing edge button directly, trying alternative approach');
    ...
    for (const selector of errorSelectors) {
      const errorVisible = await page.locator(selector).isVisible({ timeout: 500 }).catch(() => false);
      if (errorVisible) {
        hasError = true;
        break;
      }
    ...
    if (hasError) {
      console.log(`Edge from ${sourceNodeValue} to ${targetNodeValue} already exists or couldn't be created`);
